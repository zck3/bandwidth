...
# Bandwidth, a benchmark that measures memory bandwidth

This is the README file for my program `bandwidth`.

(C) Zack Smith, _3 at zs3 dot me_

## Overview

This utility is useful because memory bandwidth needs to
be measured to give you a clear idea of what your computer
is capable of. 

Merely relying on specs and marketing materials does not 
provide a full nor automatically true picture of how the hardware
can perform.

## How it measures bandwidth

My program `bandwidth` performs sequential and random
reads and writes as well as copy operations of varying chunk sizes. 
It generates a graph of the results.
You can deduce from the graph how well each type of memory 
is performing. 

So for instance when bandwidth writes a 256-byte chunk, 
you know this chunk will reside entirely in the L1 cache. 
Whereas a 256-kilobyte chunk will mainly reside in L2,
and a 256-megabyte chunk will mostly be in main memory.

Thus this program can help you hone in on memory-specific
cause(s) of good or bad system performance.
If you were to run a more real-life test like
video encoding and a browser benchmark, these would conceal
specific causes of better or worse performance.
...
* Release 1.14:
	* Android support using Termux, etc. Improved i386 support.
* Release 1.13:
	* More AVX-512. ARM nontemporal transfers.
* Release 1.12:
