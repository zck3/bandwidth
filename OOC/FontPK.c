...
FontPK *FontPK_init (FontPK* restrict self)
{
...
FontPK* FontPK_with (const char *name, int size)
{
	...
	strncpy (self->name, name, kMaxFontNameSize-1);
	...
	while (true) {
		...
		if (!resultingBitmap) {
			...
			printf ("Stored resultingBitmap %lx (%d bytes) into bitmaps[%d]\n", (unsigned long)resultingBitmap, bytesNeeded, encoding);
#endif
...
static void FontPK_describe (FontPK* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
...
FontPKClass *FontPKClass_init (FontPKClass* restrict class)
{
	SET_SUPERCLASS(Font);
