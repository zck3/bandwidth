...
#define DECLARE_FONT_METHODS(TYPE_POINTER) \
	...
	Char (*firstCharacter) (TYPE_POINTER);	\
	Char (*lastCharacter) (TYPE_POINTER);	\
	long (*totalCharacters) (TYPE_POINTER);\
	void* (*bitmapForCharacter) (TYPE_POINTER, Char characterCode, unsigned* width, unsigned* bytesPerRow, unsigned* bitsWide, unsigned* bitsHigh, int* xoffset, int* descent); 	\
	void (*sizeOfString) (TYPE_POINTER, String *, int* w, int* a, int* d); \
	void (*sizeOfWideString) (TYPE_POINTER, Char *, unsigned length, int* w, int* a, int* d); \
