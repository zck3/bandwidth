...
static void URL_describe (URL* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
...
static void URL_print (URL* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
	...
	fprintf (outputFile, "%s", self->url_string);
}

static String *URL_string (URL* restrict self)
{
	if (!self) {
		return NULL;
	}
...
static void URL_setCString (URL* restrict self, const char *url)
{
	if (!self) {
		return;
	}
...
static HTTPOperation URL_httpOperation (URL* restrict self)
{
	if (!self) {
...
static URLScheme URL_scheme (URL* restrict self)
{
	if (!self) {
...
static void URL_setHTTPOperation (URL* restrict self, HTTPOperation op)
{
	if (!self) {
		return;
	}
...
static MutableData *https_synchronous_request (char *url, HTTPOperation operation, uint8_t *post_payload, unsigned *return_statuscode)
{
	...
	do {
		...
		if (CURLE_OK != errorCode) {
			Log_warning_printf (__FUNCTION__, "Libcurl page fetch failed: %s", errorBuffer);
			release(pageContents);
			pageContents = NULL;
			break;
		} 
	...
	if (pageContents) {
		Log_debug_printf (__FUNCTION__, "Fetched page of length %lu, URL=%s\n", (unsigned long) $(pageContents, length), url);
	}
...
static MutableData *http_synchronous_request (char *url, HTTPOperation operation, uint8_t *post_payload, unsigned *return_statuscode)
{
	...
	if (!slash) {
		strncpy (domain, url, sizeof(domain)-1);
	} else {
		unsigned long domain_length = slash - url;
		if (domain_length > sizeof(domain)-1) {
			domain_length = sizeof(domain)-1;
		}
		memcpy (domain, url, domain_length);
		domain[domain_length] = 0;
		strncpy (rest, slash+1, sizeof(rest)-1);
	}
	...
	// XX Since this is non-HTTPS, assume port 80 is meant.
	const int port = 80;
...
static Data* URL_synchronousFetch (URL* restrict self, HTTPOperation operation, uint8_t *postData, size_t postLength, unsigned *statusReturn)
{
	if (!self) {
		return NULL;
	}
...
static Data* URL_synchronousGet (URL* restrict self, unsigned *statusReturn)
{
	return URL_synchronousFetch (self, HTTPOperationGet, NULL, 0, statusReturn);
}

static Data* URL_synchronousPost (URL* restrict self, uint8_t *postData, size_t postLength, unsigned *statusReturn)
{
...
URLClass* URLClass_init (URLClass *class)
{
	...
	SET_OVERRIDDEN_METHOD_POINTER(URL,print);
...
URL* URL_init (URL* restrict self)
{
...
URL *URL_withString (Any* string_)
{
	if (!string_) {
		return NULL;
	}
	...
	const char *cstring = $(string, asUTF8);
	return URL_withCString(cstring);
}
