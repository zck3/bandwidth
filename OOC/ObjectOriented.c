...
#ifdef GRATUITOUS_DEBUGGING
			printf ("Called destroy method of object %lx of class %s\n", (unsigned long)object, class->_className);
#endif
...
{
	// RULE: If objectType is NULL, this signifies that it's a non-object being allocated e.g. pixel buffer.
...
#ifdef VALGRIND
	// For whatever reasons, Valgrind is displeased by allocations less than 10 bytes.
	if (size < 10) {
		size = 10;
	}
#endif
	...
	if (!object) {
		// RULE: Report failure to allocate higher up, because an error message from here will be meaningless.
