...
FontFreetype *FontFreetype_init (FontFreetype* restrict self)
{
...
FontFreetype *FontFreetype_fromFile (const char* fontpath, int size)
{
	...
	if (!FileSystem_fileExistsWithCString (fontpath)) {
		return NULL;
	}
	//==========
	...
	for (int i = self->firstCharacter; i <= self->lastCharacter; i++) {
		...
		uint32_t *resultingBitmap = (uint32_t*) ooc_alloc_memory (bytesNeeded);
		if (!resultingBitmap) {
...
static void FontFreetype_describe (FontFreetype* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
...
FontFreetypeClass *FontFreetypeClass_init (FontFreetypeClass* restrict class)
{
        SET_SUPERCLASS(Font);
