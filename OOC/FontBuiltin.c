...
FontBuiltin* FontBuiltin_init (FontBuiltin* restrict self)
{
...
static void FontBuiltin_describe (FontBuiltin* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
...
FontBuiltinClass *FontBuiltinClass_init (FontBuiltinClass* restrict class)
{
	SET_SUPERCLASS(Font);
