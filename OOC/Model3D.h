...
// TODO Add translation, rotation properties.
...
#define DECLARE_MODEL3D_METHODS(TYPE_POINTER) \
	...
	void (*rotateX) (TYPE_POINTER, float angle); \
	void (*rotateY) (TYPE_POINTER, float angle); \
	void (*rotateZ) (TYPE_POINTER, float angle); \
	void (*rotate) (TYPE_POINTER, float x /*pitch*/, float y /*yaw*/, float z /*roll*/); \
	void (*scale) (TYPE_POINTER, float s); \
	void (*clear) (TYPE_POINTER); \
	bool (*writeSTL) (TYPE_POINTER, const char* path, const char* name); \
	bool (*readSTL) (TYPE_POINTER, const char* path); \
	void (*addCube) (TYPE_POINTER, float sideLength, RGBA);\
	void (*addSphere) (TYPE_POINTER, float diameter, unsigned steps, RGBA);\
	void (*addCylinder) (TYPE_POINTER, float height, float diameter, unsigned steps, RGBA);\
	void (*addCone) (TYPE_POINTER, float height, float diameter, unsigned steps, RGBA);
