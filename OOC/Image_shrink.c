...
static void bresenham_init (BresenhamInfo* restrict bi, int target_size, int original_size)
{
	if (!bi) {
		return;
	}
...
static int bresenham_get (BresenhamInfo* restrict bi)
{
	if (!bi) {
		return 0;
	}
...
Image* Image_shrink (Image* restrict self, unsigned target_w, unsigned target_h)
{
	if (!self) {
		return NULL;
	}
