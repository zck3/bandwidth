...
static void Image_describe (Image* restrict self, FILE *file)
{
	if (!self) {
		return;
	}
...
Image* Image_init (Image* restrict self)
{
 ...
 *-------------------------------------------------------------------------*/
static RGB Image_pixelAt(Image* restrict self, int x, int y)
{
	if (!self || x<0 || y<0) {
		return 0;
	}
 ...
 *-------------------------------------------------------------------------*/
static Size Image_size (Image* restrict self)
{
	Size size;
	size.width = 0;
	size.height = 0;
...
static uint8_t *Image_pixelRowAddress (Image* restrict self, int y)
{
	if (!self) {
		return NULL;
	}
 ...
 *-------------------------------------------------------------------------*/
static bool Image_writeBMP24 (Image* restrict self, const char *path)
{
	if (!self || !path) {
		return false;
	}
 ...
 *-------------------------------------------------------------------------*/
static bool Image_readTIFF (Image* restrict self, const char *path)
{
#ifdef HAVE_TIFF
	...
	bool needColorFlip = !TIFFIsBigEndian (tiff);
	...
	do {
		...
		if (TIFFGetField (tiff, TIFFTAG_SAMPLESPERPIXEL, &samples)) {
			depth *= samples;
			if (depth != 24 && depth != 32 && depth != 16 && depth != 8) {
				Log_warning_printf (__FUNCTION__, "only 32-bit TIFFs are supported at present.");
				ok = false;
			}
		} 
	}
	...
	Log_debug_printf (__FUNCTION__, "TIFF information: %s, width %lu, height %lu, depth %u", path, width, height, depth);
	...
	RGBA *rasterTop = (RGBA*) Image_pixelRowAddress (self, 0);
	RGBA *rasterBottom = (RGBA*) Image_pixelRowAddress (self, height-1);
	for (size_t y = 0; y < height/2; y++) {
		for (size_t x = 0; x < width; x++) {
			...
			if (needColorFlip) {
				RGBA red, green, blue;
				red = colorTop & 0xff;
				green = colorTop & 0xff00;
				blue = (colorTop >> 16) & 0xff;
				colorTop = (red << 16) | green | blue;
				red = colorBottom & 0xff;
				green = colorBottom & 0xff00;
				blue = (colorBottom >> 16) & 0xff;
				colorBottom = (red << 16) | green | blue;
			}
 ...
 *-------------------------------------------------------------------------*/
static bool Image_writeTIFF (Image* restrict self, const char *path)
{
#ifdef HAVE_TIFF
	if (!self || !path) {
		return false;
	...
	verifyCorrectClassOrSubclass(self,Image);
	if (!self->width) { 
		warning(__FUNCTION__, "Zero width image."); 
		return false;
	}
	if (!self->height) { 
		warning(__FUNCTION__, "Zero height image."); 
		return false;
	}
	//----------
...
MAKE_GETTER_SCALAR(Image,unsigned,width);
MAKE_GETTER_SCALAR(Image,unsigned,height);
 ...
 *-------------------------------------------------------------------------*/
static bool Image_writeBMP (Image* restrict self, const char *path)
{
	if (!self) {
		return 0;
	}
...
static uint8_t *Image_pixels (Image* restrict self)
{
	if (!self) {
		return NULL;
	}
...
static bool Image_equals (Image* restrict self, void *other_)
{ 
	if (!self || !other_) {
		return false;
	}
...
Image *Image_fromJPEGFile (const char *path)
{
	...
	Log_debug_printf (__FUNCTION__, "JPEG (%u x %u, depth %d).", (unsigned)width, (unsigned)height, nComponents);
	Log_debug_printf (__FUNCTION__, "JPEG rowStride %u bytes", rowStride);
