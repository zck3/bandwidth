...
static void Double_print (Double* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}
	...
	fprintf (outputFile, "%g", self->floatingPoint);
}

static void Double_describe (Double* restrict self, FILE *outputFile) 
{ 
	if (!self) {
		return;
	}	
...
static long long Double_asLongLong (Double* restrict self)
{ 
	if (!self) {
		return 0;
	}
...
static double Double_asDouble (Double* restrict self)
{ 
	if (!self) {
		return 0;
	}
...
static bool Double_asBool (Double* restrict self)
{ 
	if (!self) {
		return false;
	}
...
Double* Double_init (Double* restrict self)
{
...
static unsigned Double_hash (Double* restrict self)
{
	if (!self) {
		return 0;
	}
...
static int Double_compare (Double* restrict self, Any *other_)
{
	if (!self || !other_) {
		return 0;
	}
	verifyCorrectClass(self,Double);
	verifyCorrectClass(other_,Double);
...
static bool Double_equals (Double* restrict self, void *other_) 
{ 
	if (!self || !other_) {
		return false;
	}
	...
	verifyCorrectClass(other,Double);
...
DoubleClass* DoubleClass_init (DoubleClass* restrict class)
{
