...
void MutableImage_destroy (Any* self_)
{
	...
	releaseAndClear (self->croppingStack);
	releaseAndClear (self->animations);

	Image_destroy (self);
}

static void MutableImage_describe (MutableImage* restrict self, FILE *file)
{
	if (!self) {
		return;
	}
...
MutableImage* MutableImage_init (MutableImage* restrict self)
{
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_putPixel (MutableImage* restrict self, int x, int y, RGB rgb)
{
	if (!self || x<0 || y<0) {
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawHorizontalLine (MutableImage* restrict self, int x0, int x1, int y, RGB rgb)
{
	verifyCorrectClass(self,MutableImage);
	if (x0 > x1) {
		int tmp = x1;
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_fillRectangle (MutableImage* restrict self, int x, int y, int w, int h, RGB rgb)
{
	if (!self) { 
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawVerticalLine (MutableImage* restrict self, int x, int y0, int y1, RGB rgb)
{
	verifyCorrectClass(self,MutableImage);
	if (y0 > y1) {
		int tmp=y1;
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_line_core (MutableImage *image, 
			int x0, int y0, int x1, int y1, RGB rgb,
			int dashed)
{
	// Transparent line
	if ((rgb >> 24) == 0xff) {
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawLine (MutableImage* restrict self, int x0, int y0, int x1, int y1, RGB rgb)
{
	verifyCorrectClass(self,MutableImage);
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawDashedLine (MutableImage* restrict self, int x0, int y0, int x1, int y1, RGB rgb)
{
	if (!self) { 
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawRectangle (MutableImage* restrict self, int x, int y, int w, int h, RGB rgb)
{
	if (!self) { 
		return;
	}
 ...
 * Purpose:	Fills a rectangle with a color.
 *-------------------------------------------------------------------------*/
static void MutableImage_fillRect (MutableImage* restrict self, Rect r, RGB rgb)
{
	if (!self) { 
		return;
	}
 ...
 * Purpose:	Fills a rectangle with a color.
 *-------------------------------------------------------------------------*/
static void MutableImage_fillRectUsingVerticalGradient (MutableImage* restrict self, 
		Rect rect, RGB upper, RGB lower)
{
	if (!self || !self->pixels) {
		return;
	}
 ...
 * Purpose:	Draws a rectangle with a color.
 *-------------------------------------------------------------------------*/
static void MutableImage_drawRect (MutableImage* restrict self, Rect r, RGB rgb)
{
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_rotate (MutableImage* restrict self, MutableImageRotationAngle angle)
{
	if (!self) { 
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_fillWithColor (MutableImage* restrict self, RGB color)
{
	if (!self) { 
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_clear (MutableImage* restrict self)
{
	verifyCorrectClass(self,MutableImage);
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_grayscale (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
...
/*---------------------------------------------------------------------------
 * Name:	MutableImage_flipVertially
 * Purpose:	Flips an image vertially.
 *-------------------------------------------------------------------------*/
static void MutableImage_flipVertially (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
	...
	RGBA *rasterTop = (RGBA*) self->pixels;
	RGBA *rasterBottom = (RGBA*) self->pixels + width * (height-1);
	for (size_t y = 0; y < height/2; y++) {
		for (size_t x = 0; x < width; x++) {
			RGBA colorTop = rasterTop[x];
			RGBA colorBottom = rasterBottom[x];
			rasterTop[x] = colorBottom;
			rasterBottom[x] = colorTop;
		}
		rasterTop += width;
		rasterBottom -= width;
	}
...
/*---------------------------------------------------------------------------
 * Name:	MutableImage_exchangeRedAndBlue
 * Purpose:	Exchanges the red and blue color components of the image.
 *-------------------------------------------------------------------------*/
static void MutableImage_exchangeRedAndBlue (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_invert (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
...
/*---------------------------------------------------------------------------
 * Name:	MutableImage_drawChars
 * Purpose:	Draws characters into the image.
 * Returns:	Widths of characters drawn.
 *-------------------------------------------------------------------------*/
static int MutableImage_drawChars (MutableImage* restrict self, 
		const Char *string, unsigned length, int x, int y, Font *font, RGB color)
{
	...
	Char firstChar = $(font, firstCharacter);
	Char lastChar = $(font, lastCharacter);
...
static int MutableImage_drawCString (MutableImage* restrict self, 
		const char *cstring, int x, int y, Font *font, RGB color)
{
	...
	wide[len] = 0;
	return MutableImage_drawChars (self, (const Char*) wide, len, x, y, font, color);
}
 ...
 * Purpose:	Draws characters into the image.
 *-------------------------------------------------------------------------*/
static int MutableImage_drawString (MutableImage* restrict self, 
		String *string, int x, int y, Font *font, RGB color)
{
	if (!self || !string || !font) {
		return 0;
	}
	...
	return MutableImage_drawChars (self, characters, length, x, y, font, color);
}
 ...
 *-------------------------------------------------------------------------*/
static int MutableImage_drawMiniString (MutableImage* restrict self, 
		const char *string, int x, int y, RGB color)
{
	if (!self || !string) {
		return 0;
	}
 ...
 *-------------------------------------------------------------------------*/
static int MutableImage_miniStringWidth (MutableImage* restrict self, const char *string)
{
	if (!self || !string) {
		return 0;
	}
 ...
 *-------------------------------------------------------------------------*/
static int MutableImage_drawNarrowNumbers (MutableImage* restrict self, 
		const char *string, int x, int y, RGB color)
{
	if (!self || !string) {
		return 0;
...
static void MutableImage_drawCirclePoints (MutableImage* restrict self, 
		int cx, int cy, int dx, int dy, unsigned quadrants, RGB rgb)
{
...
static void MutableImage_drawCircle (MutableImage* restrict self, 
		int cx, int cy, unsigned radius, unsigned quadrants, RGB rgb)
{
	if (!self) { 
		return;
	}
...
static void MutableImage_fillCircle (MutableImage* restrict self, 
		int cx, int cy, unsigned radius, unsigned quadrants, RGB color)
{
	if (!self || !radius) {
		return;
	}
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_drawRoundedRectangle (MutableImage* restrict self, 
		int x0, int y0, int width, int height, int radius, RGB color)
{
 ...
 *-------------------------------------------------------------------------*/
static void MutableImage_fillRoundedRectangle (MutableImage* restrict self, 
		int x0, int y0, int width, int height, int radius, RGB color)
{
	if (!self || !radius || !width || !height) {
		return;
...
static void MutableImage_drawPixelsAt (MutableImage* restrict self, Any* pixels, Point point, unsigned pixelCount)
{
...
static void MutableImage_putImageAt (MutableImage* restrict self, Any *image_, Point point)
{
	if (!self || !image_) {
		return;
	}
	...
	int rightEdge = point.x + iw - 1;
	int bottomEdge = point.y + ih - 1;
...
static void MutableImage_drawTestPattern (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
...
static void reallocate_pixels (MutableImage* restrict self, unsigned width, unsigned height)
{
	if (!self) { 
		return;
	}
	...
	if (width > MAX_DIMENSION || height > MAX_DIMENSION) {
		Log_warning (__FUNCTION__, "Image dimension too large.");
		return;
	}
	...
	$(self->croppingStack, removeAll);

	// RULE: Always set the dimensions, but only allocate pixels if both dimensions are nonzero.
	self->width = width;
	self->height = height;
	...
	if (width > 0 && height > 0) {
		...
		if (!self->pixels) {
			Log_perror(__FUNCTION__, "Unable to allocate pixel buffer.");
		} else {
...
static MutableImage* MutableImage_initWithSize (MutableImage* restrict self, unsigned width, unsigned height)
{
	ENSURE_CLASS_READY(MutableImage);
	...
	if (self) {
		...
		// RULE: Even if the resize fails, still return a 0x0 image.
	}
	return self;
}

static void MutableImage_resize (MutableImage* restrict self, unsigned width, unsigned height)
{
	if (!self) { 
		return;
	}
...
static void MutableImage_clearIsModified (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
...
static bool MutableImage_isModified (MutableImage* restrict self)
{
	if (!self) { 
		return false;
	}
...
static void MutableImage_startAnimation (MutableImage* restrict self, Any *animation)
{
}

static void MutableImage_continueAnimations (MutableImage* restrict self)
{
}

static bool MutableImage_pushCrop (MutableImage* restrict self, Rect crop)
{
	if (!self) { 
		return false;
	}
...
static void MutableImage_popCrop (MutableImage* restrict self)
{
	if (!self) { 
		return;
	}
...
MutableImageClass* MutableImageClass_init (MutableImageClass *class)
{
	...
	SET_METHOD_POINTER(MutableImage, drawChars);
	...
	SET_METHOD_POINTER(MutableImage, flipVertially);
	SET_METHOD_POINTER(MutableImage, exchangeRedAndBlue);
