/*============================================================================
  BenchmarkLoong, an Object-Oriented C benchmark for ppc64le.
...
static void BenchmarkLoong_destroy (Any* self)
{
	if (!self) {
		return;
	}
	verifyCorrectClass(self,BenchmarkLoong);
}

static void BenchmarkLoong_describe (BenchmarkLoong* self, FILE *outputFile)
{
        if (!self) {
                return;
	}
...
//----------------------------------------------------------------------------
// Name:	BenchmarkLoong_write
// Purpose:	Performs write on chunk of memory of specified size.
//----------------------------------------------------------------------------
static long BenchmarkLoong_write (BenchmarkLoong *self, unsigned long size, BenchmarkMode mode, bool random)
{
...
//----------------------------------------------------------------------------
// Name:	BenchmarkLoong_read
// Purpose:	Performs sequential read on chunk of memory of specified size.
//----------------------------------------------------------------------------
static long BenchmarkLoong_read (BenchmarkLoong *self, unsigned long size, BenchmarkMode mode, bool random)
{
	if (size == CHECK_WHETHER_SUPPORTED) {
		switch (mode) {
		case SIZE_MAIN_REGISTER:
...
//----------------------------------------------------------------------------
// Name:	BenchmarkLoong_copy
// Purpose:	Performs sequential memory copy.
//----------------------------------------------------------------------------
static long BenchmarkLoong_copy (BenchmarkLoong *self, unsigned long size, BenchmarkMode mode)
{
	if (size == CHECK_WHETHER_SUPPORTED) {
		switch (mode) {
		case SIZE_MAIN_REGISTER:
...
BenchmarkLoongClass* BenchmarkLoongClass_init (BenchmarkLoongClass *class)
{
	SET_SUPERCLASS(Benchmark);

	SET_OVERRIDDEN_METHOD_POINTER(BenchmarkLoong,describe);
	SET_OVERRIDDEN_METHOD_POINTER(BenchmarkLoong,destroy);

	SET_METHOD_POINTER(BenchmarkLoong,read);
...
BenchmarkLoong *BenchmarkLoong_init (BenchmarkLoong *self)
{
	ENSURE_CLASS_READY(BenchmarkLoong);
